...
jQuery(".reply-question-thumbnail").on("change", function(){
    ...
    if (fileLength > 0) {
		if (file[0].size > imageSizeLimit) {
			showWarningmessage("L'image ne peut pas dépasser 3MB", "Admin Action");
		}  else {
            jQuery(".reply-question-thumbnail-label small").html("Question Media sélectionné");
		}
	}
});
...
jQuery(".reply-question-form").on("submit", function(event) {
    ...
    if (questionContent == "") {
        showMessage("Admin Action", "S'il vous plaît entrer le contenu de la question", "warning");
    }  else {
...
jQuery(".close-question").on("click", function(event) {
    ...
    confirmClose = confirm("Êtes-vous sûr de vouloir ferMoir cette question?");
    if (confirmClose) {
        ...
        success: function(response) {
            if (response.success) {
            	...
            	if (response.data.length > 0) {
                    template += `
            		    <h5 class='text-center'><b>Nouvelles Questions</b></h5><br>
    ...
    jQuery.ajax({
        ...
        success: function(response) {
            ...
            if (response.success) {
            	...
            	if (response.data.length > 0) {
                    template += `
            		    <h5 class='text-center'><b>Questions Répondues</b></h5><br>
