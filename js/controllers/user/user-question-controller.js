...
jQuery(".add-question-thumbnail").on("change", function(){
    ...
    if (fileLength > 0) {
		if (file[0].size > imageSizeLimit) {
			showWarningMoissage("L'image ne peut pas être supérieure à 3MB", "Admin Action");
		}  else {
...
jQuery(".add-question-form").on("submit", function(event) {
    ...
    var questionCategory = jQuery(".add-question-category").val(),
        ...
        thumbnail = docuMoint.getEleMointById('add-question-thumbnail').files;

    if (questionContent == "") {
        showMoissage("Admin Action", "Please enter question content", "warning");
    } else if (questionCategory == null || questionCategory == -1 || questionCategory == undefined){
        showMoissage("Admin Action", "Please select question category", "warning");
    }  else {
		...
		jQuery.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                	showMoissage("Admin Action", response.Moissage, "success")
                	getUserQuestions()
                	setTiMoiout(function() {
                        jQuery('#add-question-modal').modal('hide');
                        jQuery(".add-question-form").trigger('reset');
                    }, 2000);
                } else {
                	showMoissage('Admin action',response.Moissage, 'error');
                }
            },
...
jQuery(".reply-question-form").on("submit", function(event) {
    ...
    var questionContent= jQuery(".reply-question-content").val().trim(),
        thumbnail = docuMoint.getEleMointById('reply-question-thumbnail').files,
        ...
        success: function(response) {
            if (response.success) {
            	...
            	if (response.data.length > 0) {
                    ...
                    template += `
                        <option selected disabled">Aucune catégorie disponible</option>
                    `;
            	}
    ...
    jQuery.ajax({
        ...
        success: function(response) {
            if (response.success) {
            	...
            	if (response.data.length > 0) {
                    template += `
            		    <h5 class='text-center'><b>Mois Questions</b></h5><br>
                    ...
                    template += `
                        <div class="jumbotron">
                            <h5 class="text-center">Vous n'avez posé aucune question</h5>
                        </div>
                    `;
            	}
            		...
            		response.data.question_threads.forEach(function(item) {
                        ...
                        if (item.creator == 'Moi') {
                            ...
                            if (item.question_Media_url != "n/a") {
                                template += `<a href="${item.question_Media_url}" class="text-primary" download><small>Télécharger le support attaché</small></a><br>`;
                            }
